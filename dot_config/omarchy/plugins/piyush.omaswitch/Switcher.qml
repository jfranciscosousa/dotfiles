...
// Wayland toplevel handle. One live stream, not one per window. The preview
// pane always reserves the same space so changing its capture source cannot
// resize the switcher while the next frame is loading.
...
Item {
  ...
  // Keep all layout dimensions independent of window count, capture readiness,
  // and the selected window's aspect ratio.
  ...
  readonly property int previewFrameWidth: Math.max(1, Math.min(Style.space(580), root.previewWidth))
  readonly property int previewFrameHeight: Math.max(1, Math.min(Style.space(360), root.contentHeight))
  ...
  PanelWindow {
    ...
    BorderSurface {
      ...
      Row {
        ...
        // Keep this pane mounted at a stable size while ScreencopyView changes
        // sources. Its hasContent state briefly resets for every new window.
        BorderSurface {
          visible: true
          ...
          Item {
            width: root.previewFrameWidth
            height: root.previewFrameHeight
            anchors.centerIn: parent
