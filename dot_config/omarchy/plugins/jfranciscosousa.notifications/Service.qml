...
Item {
  ...
  // Keep notification history until the user clears it. Limit popup replay so
  // the show-history action does not open the complete archive at once.
  readonly property int historyLimit: -1
  readonly property int historyReplayLimit: 10
  ...
  // image copies included. A negative limit keeps all history. Callers set
  // $hist, $limit and $imgs first.
  readonly property string trimHistoryScript:
    "(( limit < 0 )) || { ls -1 \"$hist\" 2>/dev/null | sort -n | head -n \"-$limit\" | while IFS= read -r stale; do rm -f \"$hist/$stale\" \"$imgs/${stale%.json}\"-*; done; }"
